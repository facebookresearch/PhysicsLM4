...
# - added Canon support (put them into kv_cache) for text generation
# - kv_cache moved from Attention to TransformerBlock (since Canon layers need it)
# - removed sequence packing capability during generation (since my simple Canon implementation doesn't support advanced attention masking)
#   - (as a result) also default to sdpa attention
# - removed bos_token from def generate(self, prompts):
#   - this ensures that when evaluating using lm_eval, it matches their official HFLM implementation
#   - (other factors, such as bf16 vs mixed bf16, can still create discrepency between Lingua's lm_eval and the official lm_eval (HFLM)
#
...
class KVCache(nn.Module):
    # Zeyuan's edit note: adding module as parameter to support Canon layer conv_states
    def __init__(self, bsz, seqlen, n_heads, head_dim, dtype, device, module):
        super().__init__()
        shape = (bsz, seqlen, n_heads, head_dim)
        #print("Init KVCache with shape:", shape, "dtype:", dtype, "device:", device)
        ...
        self.print_option = False

        # conv_state: (batch, dim, state_len), where state_len >= width - 1
        if module.canonA is not None:
            self.register_buffer("canonA", torch.zeros((bsz, module.canonA.weight.shape[0], module.canonA.weight.shape[2]), dtype=dtype, device=device))
        if module.canonC is not None:
            self.register_buffer("canonC", torch.zeros((bsz, module.canonC.weight.shape[0], module.canonC.weight.shape[2]), dtype=dtype, device=device))
        if module.attention.canonB is not None:
            self.register_buffer("canonB", torch.zeros((bsz, module.attention.canonB.weight.shape[0], module.attention.canonB.weight.shape[2]), dtype=dtype, device=device))
        if module.feed_forward.canonD is not None:
            self.register_buffer("canonD", torch.zeros((bsz, module.feed_forward.canonD.weight.shape[0], module.feed_forward.canonD.weight.shape[2]), dtype=dtype, device=device))
    ...
    def reset(self):
        self.k_cache.zero_()
        self.v_cache.zero_()
        # Zeyuan's edit note: reset Canon layers if they exist
        if hasattr(self, "canonA"):
            self.canonA.zero_()
        if hasattr(self, "canonB"):
            self.canonB.zero_()
        if hasattr(self, "canonC"):
            self.canonC.zero_()
        if hasattr(self, "canonD"):
            self.canonD.zero_()
        self.offset = 0
