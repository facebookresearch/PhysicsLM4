...
# - Added Canon layers (all A,B,C,D points)
# - Integrated z-loss (appeared in ../apps/main/transformer.py file)
# - Added QK normalization
# - Added RoPE partial dimension support
# - kv_cache is moved from Attention to TransformerBlock (since Canon layers need it)
#
...
from .canon_helper import create_canon, apply_canon
...
def cross_entropy(pred, target, z_loss, **kwargs):
    if z_loss:
        loss = F.nll_loss(
        ...
        return (loss + 0.0001 * loss.square()).mean()
    else:
        return F.nll_loss(
            F.log_softmax(pred.flatten(end_dim=-2).float(), -1),
            target.flatten(end_dim=-1),
...
class Attention(nn.Module):
    def __init__(
        ...
        if args.qk_norm:
            self.q_norm = RMSNorm(n_heads * head_dim, eps=args.norm_eps)
            self.k_norm = RMSNorm(n_kv_heads * head_dim, eps=args.norm_eps)
        else:
            self.q_norm = None
            self.k_norm = None
    ...
    def forward(
        ...
        if self.canonB is not None:
            qkv = apply_canon('canonB', self.canonB, torch.cat([xq, xk, xv], dim=-1), kv_cache, None)
            xq, xk, xv = qkv.split([self.n_heads * self.head_dim, self.n_kv_heads * self.head_dim, self.n_kv_heads * self.head_dim], dim=-1)
            #assert len(xq.shape) == 3, f"{xq.shape=} {xk.shape=} {xv.shape=}"
        ...
        if self.rope_dim is None:
            ...
            xq_rot, xk_rot = apply_rotary_emb(xq[..., : self.rope_dim],
                                              xk[..., : self.rope_dim],
                                              1,
                                              freq_cis[0:seq_len])
            xq = torch.cat((xq_rot, xq[..., self.rope_dim:]), dim=-1)
            xk = torch.cat((xk_rot, xk[..., self.rope_dim:]), dim=-1)
        ...
        if kv_cache is not None:
            xk, xv = kv_cache.update(xk, xv, tok_idx)
        ...
        if attn_impl == "flex_attention":
            ...
            xq, xk, xv = map(lambda e: e.transpose(1, 2), (xq, xk, xv))
            assert mask is None or isinstance(mask, (str, torch.Tensor)), f"{mask=}"
...
class FeedForward(nn.Module):
    ...
    def forward(self, x: torch.Tensor, kv_cache) -> torch.Tensor:
        ...
        if self.canonD is not None:
            dd = apply_canon('canonD', self.canonD, torch.cat([x1, x3], dim=-1), kv_cache, None)  # doesn't allow any attention_mask
            x1, x3 = dd.chunk(2, dim=-1)
...
class TransformerBlock(nn.Module):
    ...
    def init_weights(self, init_std=None, factor=1.0):
        ...
        if self.canonC is not None:
            self.canonC.reset_parameters()
        if self.attention.canonB is not None:
            self.attention.canonB.reset_parameters()
        if self.feed_forward.canonD is not None:
            self.feed_forward.canonD.reset_parameters()
...
class BaseTransformer(nn.Module):
    def __init__(self, args: BaseTransformerArgs):
        ...
        self.rope_embeddings = RotaryEmbedding(
            ...
            head_dim=args.rope_dim or head_dim,
